...
def SolveTSP(matrix, iBoundary):
    ...
    print("FINAL CONFIGURATION RANK:", rank)
...
if __name__ == '__main__':
    ...
    factorials = PreComputeFactorials(E)
    boundary1 = Boundary(n, 1, factorials)
    boundary2 = Boundary(n, 2, factorials)
    rank = 0
    # CHOOSE A BOUNDARY FOR TESTING
    boundaryChoice = 1
    if boundaryChoice == 1:
        print("CHOSEN BOUNDARY: BOUNDARY 1")
        print("BOUNDARY 1:", boundary1)
        rank = random.randint(0, boundary1 - 1)
    elif boundaryChoice == 2:
        print("CHOSEN BOUNDARY: BOUNDARY 2")
        print("BOUNDARY 1:", boundary1)
        print("BOUNDARY 2:", boundary2)
        rank = random.randint(boundary1, boundary2)
    print("INITIAL CONFIGURATION RANK:", rank)
    # RANDOM PERMUTATION OF EDGE WEIGHTS
    edges = UnrankPermutation(rank, E, factorials)
    # RANDOM ARITHMETIC PROGRESSION
    firstTerm = random.randint(1, 10)
    commonDifference = random.randint(1, 10)
    print("ARITHMETIC SERIES FIRST TERM:", firstTerm)
    print("ARITHMETIC SERIES COMMON DIFFERENCE:", commonDifference)
    for i in range(E):
        edges[i] = (edges[i] * commonDifference) + firstTerm
    matrix = [[0 for x in range(n)] for y in range(n)]
    # FILL MATRIX WITH DIAGONAL VECTORIZATION
    edge = 0
    for i in range(1, n + 1):
        y = 0
        x = i
        for j in range(i, n):
            # COLLECT EDGE
            matrix[y][x] = edges[edge]
            matrix[x][y] = edges[edge]
            edge += 1
            # TRAVERSE THE MATRIX DIAGONALLY
            y += 1
            x += 1
    # RANDOM PERMUTATION OF VERTEX LABELS
    vertexPermutationRank = random.randint(0, factorials[n] - 1)
    print("VERTEX PERMUTATION RANK:", vertexPermutationRank)
    permutedVertices = UnrankPermutation(vertexPermutationRank, n, factorials)
    print("PERMUTED VERTICES:", permutedVertices)
    # BUILD ADJACENCY MATRIX OF PERMUTED VERTICES
    permutedMatrix = [[0 for x in range(n)] for y in range(n)]
    # TRAVERSE THE MATRIX WITH LEFT TO RIGHT VECTORIZATION
    edge = 0
    for i in range(n):
        for j in range(i + 1, n):
            oldI = permutedVertices[i]
            oldJ = permutedVertices[j]
            # RELABEL MATRIX
            permutedMatrix[i][j] = matrix[oldI][oldJ]
            permutedMatrix[j][i] = matrix[oldJ][oldI]
            edge += 1
    print("PERMUTED MATRIX:")
    for row in permutedMatrix:
        print(row, end=",\n")
    # TEST AND TIME ALGORITHM
    start = timer()
    SolveTSP(permutedMatrix, boundaryChoice)
    end = timer()
    duration = end - start
    print("TSP ARITHMETIC SERIES CLASSICAL ALGORITHM ELAPSED TIME:", duration, "s")
